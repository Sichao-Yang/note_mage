...
def main(input_folder, output_folder):
    ...
    md_files = [osp.join(output_folder, x) for x in os.listdir(output_folder) if ".md" in x]
    logging.info(f"Found {len(md_files)} markdown files from {input_folder}:\n{pformat(md_files)}")
    global IMGPATTERNS
            ...
            for p in compiled_p:
                ...
                if len(res) != 0:
                    ...
                    if new_pic_path != "":
                        ...
                        if not pic_path_abs in moved_list:
...
if __name__ == "__main__":
    logger = get_logger(filename="imgpath_correct.log", verb_level="info", method="w2file")
